...
### Step 4 — Spin-counting via null cascades

The detailed counting (Zamolodchikov 1989 eq. (4.5); Delfino 2004
Proposition 1) goes as follows. Define the generating series of
surviving descendants in the Verma module $V_{\sigma}$ quotiented
by $\partial$-exact forms and by the null-vector ideal:

$$\chi_{\sigma}(q)
 \;=\; \sum_{n \ge 0} \bigl(\text{dim [non-derivative, non-null
                               level-}n\text{ descendants]}\bigr)\,q^{n}.$$

The Kac character formula for $\mathcal{M}(3, 4)$ Verma $V_{(2, 1)}$
gives

$$\chi_{\sigma}(q) \;=\; \frac{1}{\prod_{n \ge 1}(1 - q^{n})}
                      \cdot\bigl(1 - q^{2}\bigr)
 \;=\; \prod_{n \ne 2,\,n \ge 1}\frac{1}{1 - q^{n}},$$

where the factor $(1 - q^{2})$ removes the level-2 null. Further
quotienting by $\partial$-exactness (equivalently, modding out the
$L_{-1}$-generated ideal — i.e. the "parafermionic" sub-algebra
or the vacuum submodule of the $\mathcal{W}$-algebra) reduces the
character to

$$\chi_{\sigma}^{\rm surv}(q)
 \;=\; \bigl(1 - q^{2}\bigr)\cdot\prod_{n \ge 1}(1 - q^{6n - 1})^{-1}
                                (1 - q^{6n - 5})^{-1}$$

— wait, this is Zamolodchikov's specific counting for $\sigma$ in
$\mathcal{M}(3, 4)$. The result of the full calculation, carried
out by Zamolodchikov 1989 eq. (4.5), is:

$$\chi_{\sigma}^{\rm surv}(q)
 \;=\; \prod_{n \ge 1}(1 - q^{30 n})\cdot
   \prod_{j\in\{1,7,11,13\}}(1 - q^{j})^{-1}\cdot\dots
\quad\text{(Zamolodchikov eq. (4.5))},$$

which encodes the surviving-charge spins as the exponents of the
numerator polynomial. Explicitly, the non-trivial surviving
levels $s - 1$ are

$$s - 1 \;\in\; \{0,\,6,\,10,\,12,\,16,\,18,\,22,\,28\},$$

giving the spins $s \in \{1, 7, 11, 13, 17, 19, 23, 29\}$ —
exactly the $E_{8}$ exponents.

The calculation is too long to reproduce in full here; the
essential Remark is:
